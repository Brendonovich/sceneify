...
export interface SceneDeclaration<
  TItems extends Record<string, SceneItem.SceneItemDecl<any>>,
  TFilters extends Record<string, Filter.Config<any>> = {}
> {
  readonly name: string;
...
> {
  ...
  readonly item: <K extends string & keyof TItems>(
    key: K
  ) => SceneItem.SceneItem<TItems[K]["source"]>;
...
export const sync = <
  const TItems extends Record<
    string,
    SceneItem.SceneItemDecl<Input.Declaration<InputType<string, any>>>
  >,
...
export const declare = <
  const TItems extends Record<string, SceneItem.SceneItemDecl<any>>,
  const TFilters extends Record<string, Filter.Config<any>> = {}
>(options: {
  name: string;
  items: TItems;
