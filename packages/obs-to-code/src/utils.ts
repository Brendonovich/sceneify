...
/**
 * Infers an Effect Schema expression from a value.
 */
export function inferSchema(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    ...
    const elementSchemas = [...new Set(value.map(inferSchema))];
    const elementSchema =
      elementSchemas.length === 1
        ? elementSchemas[0]
        : `Schema.Union(${elementSchemas.join(", ")})`;
    ...
    const fields = entries
      .map(([key, val]) => {
        ...
        return `${JSON.stringify(key)}: ${
          val === undefined ? `Schema.optional(${schema})` : schema
        }`;
      })
      .join(", ");
